import type { DataSolutionsConfig, HeadingPart, NumberedStep } from '../../data/types'

export default function DataProcess({ data }: { data: DataSolutionsConfig['process'] }) {
  if (!data) return null
    ...
    <section className="ds-process" aria-label="How we work">
      <div className="ds-process-inner">
        <header className="ds-process-header">
          ...
          <h2>
            {headingParts.map((part: HeadingPart) => (
        ...
        <div className="ds-process-steps">
          {data.steps.map((step: NumberedStep) => (
