...
const SERVICE_META: Record<string, { name: string; title: string; description: string; image: string }> = {
  'digital-growth': {
    name: 'Digital Growth',
...
export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params
  const service = SERVICE_PAGES[slug as keyof typeof SERVICE_PAGES]
  const meta = SERVICE_META[slug]
  ...
  if (slug === 'digital-growth') {
    const service = SERVICE_PAGES['digital-growth']
    return (
  ...
  if (slug === 'data-solutions') {
    const service = SERVICE_PAGES['data-solutions']
    return (
      <main className="service-page">
        {structured}
  ...
  const web = SERVICE_PAGES['web-software-development']
  return (
    <main className="service-page">
      {structured}
      <WebSoftwareHero data={web.hero} />
      <ServiceShowcaseTabs tabs={web.showcaseTabs} />
      <WebSoftwareTech data={web.technologies} />
      <WebSoftwareProcess data={web.process} />
      <Faqs items={web.faqs} />
      <Contact />
    </main>
  )
